...
class DecoderTransformer_v3(BaseDecodeHead):
    """
    Transformer Decoder
    """
    def __init__(self, in_channels = 3, channels=256, in_channel_list = [32, 64, 128, 256], input_transform='multiple_select',
                 in_index=[0, 1, 2, 3], align_corners=True,
                 embedding_dim= 64, output_nc=2,
                 decoder_softmax = False, feature_strides=[2, 4, 8, 16], loss_decode=None):
        super(DecoderTransformer_v3, self).__init__(in_channels=in_channels, channels=channels, num_classes=output_nc)
        # assert
        assert len(feature_strides) == len(in_channel_list)
        ...
        c1_in_channels, c2_in_channels, c3_in_channels, c4_in_channels = self.in_channel_list
        ...
        self.linear_fuse = nn.Sequential(
            nn.Conv2d(in_channels=self.embedding_dim *len(in_channel_list), out_channels=self.embedding_dim,
                      kernel_size=1),
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg):
        ...
        print(seg_logits[0].shape)
        print("seg_logits:", seg_logits)
        seg_logits = torch.cat([torch.unsqueeze(xi, dim=0) for xi in seg_logits], dim=0)
...
class DecoderTransformerV3(DecoderTransformer_v3):
    def __init__(self, in_channels, channels, **kwargs):
        super(DecoderTransformerV3, self).__init__(in_channels, channels, **kwargs)